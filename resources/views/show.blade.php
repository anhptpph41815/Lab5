...
<form action="{{route('store')}}" method="POST" enctype="multipart/form-data" >
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="title" name="title" placeholder="Title movie" value="{{$movie->title}}" disabled>
      </div>
      <div class="mb-3">
        <label for="exampleFormControlInput1" class="form-label">Poster</label>
        <img src="{{asset('/Storage/' .$movie->poster)}}" alt="" width="30%" disabled>
      </div>
      <div class="mb-3">
        <label for="exampleFormControlTextarea1" class="form-label" >Intro Movie</label>
        <textarea class="form-control" id="intro" name="intro" rows="3" disabled>{{$movie->intro}}</textarea>
      </div>
      <div class="mb-3">
        <label for="exampleFormControlInput1" class="form-label">Release date</label>
        <input type="date" class="form-control" id="release" name="release_date" placeholder="release movie" disabled value="{{$movie->release_date}}">
      </div>

      <div class="mb-3">
        <label for="exampleFormControlInput1" class="form-label">Genre</label>
        <input type="text" class="form-control" id="release" name="release_date" placeholder="release movie" disabled value="{{$movie->genre->name}}">
      </div>
      </select>
      {{-- <div class="mb-3">
